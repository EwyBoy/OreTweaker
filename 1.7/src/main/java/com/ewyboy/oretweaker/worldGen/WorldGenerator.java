...
public class WorldGenerator implements IWorldGenerator {
    ...
    private Block[] blocks = {Blocks.coal_ore, Blocks.iron_ore, Blocks.gold_ore, Blocks.diamond_ore, Blocks.redstone_ore, Blocks.dirt, Blocks.gravel, Blocks.emerald_ore};
        ...
        for (int ore = 0; ore < Reference.OreNames.OreNames.length; ore++) {
            if (disableOres[ore] && enableCustomOreGeneration[ore]) {
                for (int i = 0; i < spawnRates[ore]; i++) {
                    ...
                    new WorldGenMinable(blocks[ore], veinSize).generate(world, random, xCoord, veinYLevel, zCoord);
                }
            }
        }
    }
